...
public abstract class SyncPDSSolver<Stmt extends Location, Fact, Field extends Location, W extends Weight> {
    ...
    private final Multimap<Node<Stmt, Fact>, SyncStatePDSUpdateListener<Stmt, Fact>> reachedStateUpdateListeners = HashMultimap
            .create();
    ...
    public SyncPDSSolver(INode<Fact> initialCallNode, INode<Node<Stmt, Fact>> initialFieldNode,
            final boolean useCallSummaries, NestedWeightedPAutomatons<Stmt, INode<Fact>, W> callSummaries,
    ...
    private class FieldAddEpsilonToInitialStateListener extends WPAStateListener<Field, INode<Node<Stmt, Fact>>, W> {
        ...
        public FieldAddEpsilonToInitialStateListener(INode<Node<Stmt, Fact>> state,
    ...
    private class FieldOnOutTransitionAddToStateListener extends WPAStateListener<Field, INode<Node<Stmt, Fact>>, W> {
        ...
        public FieldOnOutTransitionAddToStateListener(INode<Node<Stmt, Fact>> state,
                Transition<Field, INode<Node<Stmt, Fact>>> nestedT) {
            super(state);
    ...
    public void solve(Node<Stmt, Fact> curr, W weight) {
        ...
        Transition<Field, INode<Node<Stmt, Fact>>> fieldTrans = new Transition<Field, INode<Node<Stmt, Fact>>>(start,
                emptyField(), target);
    ...
    public void addNormalCallFlow(Node<Stmt, Fact> curr, Node<Stmt, Fact> succ) {
        addCallRule(new NormalRule<Stmt, INode<Fact>, W>(wrap(curr.fact()), curr.stmt(), wrap(succ.fact()), succ.stmt(),
                getCallWeights().normal(curr, succ)));
    }

    public void synchedEmptyStackReachable(final Node<Stmt, Fact> sourceNode,
            final EmptyStackWitnessListener<Stmt, Fact> listener) {
            ...
            public void onReachableNodeAdded(Node<Stmt, Fact> reachableNode) {
                ...
                callAutomaton.registerListener(new WPAUpdateListener<Stmt, INode<Fact>, W>() {
                    ...
                    public void onWeightAdded(Transition<Stmt, INode<Fact>> t, W w,
                            WeightedPAutomaton<Stmt, INode<Fact>, W> aut) {
                        if (t.getStart() instanceof GeneratedState)
                            return;
    ...
    public void addNormalFieldFlow(final Node<Stmt, Fact> curr, final Node<Stmt, Fact> succ) {
        if (succ instanceof ExclusionNode) {
            ...
            addFieldRule(new NormalRule<Field, INode<Node<Stmt, Fact>>, W>(asFieldFact(curr), fieldWildCard(),
                    asFieldFact(succ), exclusionFieldWildCard(exNode.exclusion()),
                    getFieldWeights().normal(curr, succ)));
            return;
        }
        addFieldRule(new NormalRule<Field, INode<Node<Stmt, Fact>>, W>(asFieldFact(curr), fieldWildCard(),
                asFieldFact(succ), fieldWildCard(), getFieldWeights().normal(curr, succ)));
    }
    ...
    public void processPop(Node<Stmt, Fact> curr, PopNode popNode) {
        ...
        if (system.equals(PDSSystem.FIELDS)) {
            ...
            if (ContextSensitive) {
                addCallRule(new PopRule<Stmt, INode<Fact>, W>(wrap(curr.fact()), curr.stmt(), wrap((Fact) location),
                        getCallWeights().pop(curr, returnSite)));
            } else {
        ...
        if (system.equals(PDSSystem.FIELDS)) {
            ...
            if (FieldSensitive) {
                if (!fieldContextReachable.contains(succ)) {
                    addFieldRule(new PushRule<Field, INode<Node<Stmt, Fact>>, W>(asFieldFact(curr), fieldWildCard(),
                            asFieldFact(succ), (Field) location, fieldWildCard(),
            ...
            addNormalFieldFlow(curr, succ);
            if (ContextSensitive) {
                addCallRule(new PushRule<Stmt, INode<Fact>, W>(wrap(curr.fact()), curr.stmt(), wrap(succ.fact()),
                        succ.stmt(), (Stmt) location, getCallWeights().push(curr, succ, (Stmt) location)));
            } else {
                addNormalCallFlow(curr, succ);
            }
    ...
    public void debugOutput() {
        ...
        System.out.println(fieldPDS.toString());
        System.out.println(fieldAutomaton.toDotString());
        System.out.println(callingPDS.toString());
